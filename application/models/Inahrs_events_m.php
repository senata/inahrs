...
class Inahrs_events_m extends CI_Model
{
    ...
    function getEventSymposium($event_id){
        $this->db->select('*');
        $this->db->from('event_symposium');
        $this->db->where('event_id', $event_id);
        $this->db->join('event_participant_type', 'event_symposium.membership_type = event_participant_type.id');
        $query = $this->db->get();
        return $query->result_array();   
    }

    // function getEventWorkshop($event_id){
    //     $this->db->where('event_id', $event_id);
    //     $query = $this->db->get('event_workshop');
    //     return $query->result_array();   
    // }

    function getEventWorkshop($event_id, $special = 0){
        $this->db->where('special_package', $special);
        $this->db->where('event_id', $event_id);
        $query = $this->db->get('event_workshop');
        return $query->result_array();   
    }
